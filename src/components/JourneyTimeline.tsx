...
function EntryCard({ entry, lang }: { entry: TimelineEntry; lang: "en" | "sv" }) {
  ...
  return (
    ...
    >
      ...
      {/* Title */}
      <p className="font-bold text-base leading-snug text-ink-900 mb-0.5">
        {entry.title[lang]}
      </p>
...
const parallelNote = {
  en: (count: number) =>
    `During 2024–2025, ${count} activities ran concurrently — two study programmes alongside two jobs. An unusually intensive period of simultaneous growth.`,
  sv: (count: number) =>
    `Under 2024–2025 pågick ${count} aktiviteter parallellt — två studieprogram vid sidan av två anställningar. En ovanligt intensiv period av lärande och arbete på samma gång.`,
};
...
export default function JourneyTimeline() {
  ...
  const eduEntries = timelineEntries
    .filter((e) => e.type === "education")
    .sort(byRecency);

  const expEntries = timelineEntries
